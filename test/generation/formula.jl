...
# @testset "randformula" begin

_alphabet = ExplicitAlphabet(["p", "q", "r", "s"])
_operators = [NEGATION, CONJUNCTION, IMPLICATION]
w = [10,1,1]

@test_nowarn [randformula(i, _alphabet, _operators) for i in 1:5]
@test_nowarn [randformula(i, _alphabet, _operators, opweights=w) for i in 1:2]
...
@test_nowarn randformula(2, my_alph, my_ops)
@test_nowarn randformula(MersenneTwister(42), 2, my_alph, my_ops)

@test_nowarn randformula(4, my_grammar)
@test_nowarn randformula(MersenneTwister(42), 4, my_grammar)

...
#     # if atompicking_mode ∉ [:uniform, :uniform_subalphabets, :weighted]
#     if !(atompicking_mode in [:uniform, :uniform_subalphabets, :weighted])
#         throw("Value for `atompicking_mode` not...")
#     end
#     rng = initrng(rng)
...
#             throw(
#                 "Mismatching numbers of alphabets "*
...
#     maxdepth >= 0 || throw("maxdepth must be greater than or equal to 0, but got $maxdepth")

#     if !isnothing(nformulas)
#         nformulas > 0 || throw("nformulas must be greater than 0, but got $nformulas")
#     end
